...
async def execute(self, ctx, parameters, args):
    ...
    if current_stream is None:
        await ctx.send(parameters['offline_message'].format(caster_name=str(ctx.message.channel)))
    else:
        delta = datetime.datetime.fromtimestamp(current_stream['started_at']) - datetime.datetime.utcnow()
        await ctx.send(parameters['online_message'].format(caster_name=str(ctx.message.channel), time=str(delta)))